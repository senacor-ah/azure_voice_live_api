...
function StepNode({ data }: NodeProps<Node<StepData>>) {
  const { label, sublabel, icon, color, color2, status, isSelected } = data
  ...
  return (
    <div className="flex flex-col items-center select-none group">
      ...
      <div
        className={`
          relative flex flex-col items-center gap-2.5 px-5 py-4 rounded-2xl
...
function fwd(
  ...
  const c = active ? color : '#d4d4d8'
  return {
    id, source: src, sourceHandle: 'src-right', target: tgt, targetHandle: 'tgt-left',
    type: 'smoothstep',
    animated: active,
    style: { stroke: c, strokeWidth: active ? 2.5 : 1.5, opacity: active ? 1 : 0.5 },
...
function ret(
  ...
  return {
    ...
    style: { stroke: c, strokeWidth: 1.5, strokeDasharray: '6 4', opacity: active ? 0.8 : 0.35 },
...
function buildGraph(
  ...
  const nodes: Node[] = [
    ...
    },
    {
      ...
      position: { x: 230, y: 0 },
      selectable: false, draggable: false,
      style: { width: 1080, height: 580, zIndex: -1, pointerEvents: 'none' } as React.CSSProperties,
    ...
    },
    {
      ...
      selectable: false, draggable: false,
      style: { width: 1080, height: 180, zIndex: -1, pointerEvents: 'none' } as React.CSSProperties,
  ...
  const stepDefs: Array<{
    ...
    { id: 'speech',         label: 'Speech Input',       sublabel: 'Spracheingabe',       icon: 'mic',            color: '#334155', color2: '#1e293b', x: 30,  y: 100 },
    { id: 'speech_out',     label: 'Speech Response',    sublabel: 'Sprachausgabe',       icon: 'speaker',        color: '#059669', color2: '#10b981', x: 30,  y: 640 },
    ...
    { id: 'rag',            label: 'RAG Pipeline',       sublabel: 'Wissensabfrage',      icon: 'search',         color: '#d97706', color2: '#f59e0b', x: 960, y: 55  },
    { id: 'fn_appointment', label: 'Terminvereinbarung', sublabel: 'Function Call',       icon: 'calendar',       color: '#7c3aed', color2: '#8b5cf6', x: 960, y: 225 },
    { id: 'fn_transfer',    label: 'Überweisung',        sublabel: 'Function Call',       icon: 'arrowleftright', color: '#db2777', color2: '#ec4899', x: 960, y: 395 },
    { id: 'kunden_api',      label: 'Kunden API',         sublabel: 'OnPrem Integration',  icon: 'plug',           color: '#ca8a04', color2: '#eab308', x: 450, y: 640 },
    { id: 'transaction_api',  label: 'Transaktions API',   sublabel: 'OnPrem Integration',  icon: 'plug',           color: '#ca8a04', color2: '#eab308', x: 850, y: 640 },
  ]
  ...
  const edges: Edge[] = [
    ...
    fwd('e4', 'agent',         'rag',            '#2563eb', on('agent')),
    fwd('e5', 'agent',         'fn_appointment', '#2563eb', on('agent')),
    fwd('e6', 'agent',         'fn_transfer',    '#2563eb', on('agent')),
    // Return: functions → agent
    ret('e7',  'rag',            'src-left', 'agent', 'tgt-top',    '#d97706', on('rag'),            'Ergebnis'),
    ret('e8',  'fn_appointment', 'src-left', 'agent', 'tgt-right',  '#7c3aed', on('fn_appointment'), 'Ergebnis'),
    ret('e9',  'fn_transfer',    'src-left', 'agent', 'tgt-bottom', '#db2777', on('fn_transfer'),    'Ergebnis'),
    // Handler → Kunden API (call via top)
    ret('e-api1', 'handler', 'src-bottom', 'kunden_api', 'tgt-top', '#475569', on('handler')),
    // Kunden API → Handler (response via right)
    ret('e-api2', 'kunden_api', 'src-right', 'handler', 'tgt-bottom', '#ca8a04', on('kunden_api'), 'Response'),
    // Überweisung → Transaktions API (call via top → bottom)
    ret('e-tx1', 'fn_transfer', 'src-bottom', 'transaction_api', 'tgt-top', '#db2777', on('fn_transfer')),
    // Transaktions API → fn_transfer (response via right)
    ret('e-tx2', 'transaction_api', 'src-right', 'fn_transfer', 'tgt-bottom', '#ca8a04', on('transaction_api'), 'Response'),
    ...
    ret('e11', 'voicelive', 'src-bottom', 'speech_out', 'tgt-top', '#059669', on('voicelive'), 'WebRTC'),
  ]
