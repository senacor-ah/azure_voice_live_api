...
export function SessionStats({
  ...
  return (
    <div className={cn("rounded-b-2xl p-4 h-full transition-all duration-300", className)}>
      <h3 className="text-sm font-semibold mb-3 flex items-center gap-2" style={{color: '#1a1a2e'}}>
        <Zap className="w-4 h-4" style={{color: '#7da0d7'}} />
        Session Stats
      </h3>
      ...
      <div className={cn("space-y-2", compact && "space-y-1.5")}>
        ...
        <div className={cn("flex items-center justify-between", compact ? "text-xs" : "text-sm")}>
          ...
          <span className="font-medium flex items-center gap-1.5">
            {isConnected && (
              <CheckCircle className={cn(compact ? "w-3 h-3" : "w-3.5 h-3.5")} style={{color: '#22c55e'}} />
            )}
